...
def prepare_optimizer_input(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Production Schedule F quantity is already arriving/available in the selected target week.
    # Therefore the optimizer must use inventory AFTER adding F Wk3, not the pre-F inventory.
    # Keep the pre-F values for debug/audit, then overwrite the standard optimizer columns
    # so every downstream allocation calculation uses New SI and New SI-SS.
    df["Original SI Before F Wk3"] = df["Sum of SI Wk3"]
    df["Original SI-SS Before F Wk3"] = df["Sum of SI-SS Wk3"]
    df["New SI"] = df["Original SI Before F Wk3"] + df["F Wk3"]
    df["New SI-SS"] = df["Original SI-SS Before F Wk3"] + df["F Wk3"]
